...
void kernel_main(void) {
    ...
    if (pages) {
        ...
        esp_printf(putc, "[HW3] Failed to allocate pages\r\n\r\n");
    }

    // HW5: IDE and FAT
    esp_printf(putc, "[HW5] Initializing IDE driver...\r\n");
    ide_init();   // assumes ide_init() is declared in ide.h / defined in ide.c
    ...
    if (fatInit() != 0) {
        ...
        // fatOpen matches your fat.h: struct file *fatOpen(const char *filename)
    ...
    idt_init();   // assumes idt_init() is declared in interrupt.h / defined in interrupt.c
